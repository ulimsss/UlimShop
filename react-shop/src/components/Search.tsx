...
const Search = () => {
  ...
  const goSearchList = (event: any) => {
    if (40 === event.keyCode) {
      ...
      if (!$next || !$next.querySelectior($searchedItem)) {
        return;  
      }
  ...
  return (
    <div className="dropdown">
      ...
      <input
        ...
        onClick={(e) => goSearchList(e)}
        className="fixed left-0 top-4 opacity-0 sm:opacity-100 sm:static sm:flex w-full input input-ghost focus:outline-0 rounded-none sm:rounded bg-gray-300 dark:bg-gray-600 !text-gray-800 dark:!text-white sm:transform-none transition-all js-searchInput -z-10"
        value={search}
      ></input>
